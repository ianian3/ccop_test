#!/usr/bin/env python3
"""
CCOP SFT 데이터 합성 생성기 V3.3
================================
V3.3 신규 온톨로지 (23 Nodes, 사칭 노드 vt_impersonation 포함)를 반영한 
수사망(Text-to-Cypher) 학습 전용 데이터셋 생성 스크립트.

특징:
  1. 사칭(impersonates) 엣지가 (객체)-[used_for]->(사칭노드)-[targets]->(기관) 2-Hop 구조로 개편됨.
  2. 공백 처리(Sanitizer) 로드 감소를 위해 정답 Cypher 문장 내 키워드 간 공백 엄격 준수.
  3. ShareGPT (sft_v33_sharegpt.json) 및 Alpaca 포맷 동시 배출

실행:
  cd /Users/iankwon/test/coop_v1.0
  python3 scripts/generate_v33_synth_sft.py --target 15000
"""
...
OUT_SHAREGPT = os.path.join(BASE_DIR, "data", "sft_v33_sharegpt.json")
OUT_ALPACA   = os.path.join(BASE_DIR, "data", "sft_v33_alpaca.jsonl")
...
TEMPLATES = [
    # 1. 신규 V3.3 2-Hop 사칭 패턴
    (
        ["번호 '{telno}'가 {org_name}을 사칭한 목록을 보여줘.", 
         "{org_name}을 사칭한 '{telno}'을 조사해 줘.",
         "기관 {org_name}을 사칭해 사용된 번호 '{telno}' 내역."],
        lambda v: sql(f"MATCH (t:vt_telno {{ telno: '{v['telno']}' }})-[r1:used_for]->(i:vt_impersonation)-[r2:targets]->(o:vt_org {{ org_name: '{v['org_name']}' }}) RETURN t, r1, i, r2, o", ["t","r1","i","r2","o"])
    ),
    (
        ["{org_name}을 사칭한 전화번호들을 다 찾아봐.", 
         "'{org_name}' 사칭에 이용된 번호 리스트."],
        lambda v: sql(f"MATCH (t:vt_telno)-[r1:used_for]->(i:vt_impersonation)-[r2:targets]->(o:vt_org {{ org_name: '{v['org_name']}' }}) RETURN t, r1, i, r2, o", ["t","r1","i","r2","o"])
    ),
...
"""
    ...
    print(f"✅ V3.3 SFT 데이터셋 {target}건 생성 완료!")
