...
사용법:
  python scripts/ingest_legal_corpus.py                       # data/legal/*.json 전체 적재(+임베딩)
...
코퍼스 포맷: [{doc_id, law_name, article_ref, title, content, source, tags[]}, ...]
...
def load_records(corpus_dir):
    ...
    for path in sorted(glob.glob(os.path.join(corpus_dir, "*.json"))):
        ...
        if not isinstance(data, list):
            ...
            continue
        print(f"  · {os.path.basename(path)}: {len(data)}건")
        records.extend(data)
    return records


def main():
    parser = argparse.ArgumentParser(description="법률 RAG 코퍼스 적재")
    ...
    with app.app_context():
        from app.services.legal_rag_service import LegalRAGService, tokenize
