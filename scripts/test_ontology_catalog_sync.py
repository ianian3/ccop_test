...
def main():
    ...
    check(f'INFERENCE_RULES = 통합 dict 13종 (현재 {len(ir)})', len(ir) == 13,
          'dict 리터럴 13종 아님 (list 회귀 또는 개수 불일치)')
    check('RelayStationDetection 단일 등재 (양쪽 중복 병합)', 'RelayStationDetection' in ir,
          f'현재 키: {sorted(ir)}')
    v37_literal = dict_keys(tree, 'INFERENCE_RULES_V37')  # 파생 뷰(comprehension)면 리터럴 미검출
    check('INFERENCE_RULES_V37은 파생 뷰 (dict 리터럴 재정의 금지)', not v37_literal,
          f'V37이 리터럴로 재이원화됨: {sorted(v37_literal)}')
