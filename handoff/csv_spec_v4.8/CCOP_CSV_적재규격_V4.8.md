...
됩니다(실측). **선택 파일이 없다고 해서 나머지가 안 들어가지는 않습니다.**

선택 7종 중에서도 더 줄일 수 있습니다.

| 선택 파일 | 언제 필요한가 |
|---|---|
| `tbl_eg_ip_use` · `tbl_eg_id_use` · `tbl_eg_id_msg` | IP·메신저 자료가 있으면 **권장** (2차년도 최다 관계) |
| `tbl_eg_loc_use` | ATM처럼 통화·이체 파일로 안 들어오는 위치가 있을 때만 |
| `tbl_vt_ip` · `tbl_vt_id` · `tbl_vt_loc` | **대개 불필요.** 관계 파일이 이 노드들을 자동으로 만들기 때문에, ASN·국가·별명 같은 **노드 자체의 속성이 따로 있을 때만** 주시면 됩니다 |

> 같은 원리로 `tbl_vt_telno`·`tbl_vt_bacnt` 도 통신사·계좌종류 같은 속성이 없으면 생략 가능합니다.
> 다만 **`tbl_vt_psn`(인물)은 `psn_id`를 쓰신다면 반드시 필요합니다** — 관계 파일의 `psn_id`를
> 이름으로 바꿀 표가 없으면 같은 사람이 `홍길동`과 `P-2026-0001`로 갈라집니다.
> (적재기가 이 경우를 경고하지만, 애초에 인물 파일을 함께 주시는 편이 안전합니다.)
...
`core/`(필수 9종)와 `optional/`(선택 7종)로 나눠 뒀고, 검증·적재 도구는 하위 폴더까지 함께 읽습니다.
