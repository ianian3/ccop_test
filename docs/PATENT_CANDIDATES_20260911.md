...
| J. **온톨로지 설계 자체** (노드25·엣지72 스키마) | ① **성립성** — 분류체계·스키마 등 정보의 구조 자체는 "자연법칙을 이용한 기술적 사상"이 아님(SW 발명 심사기준상 데이터 구조는 HW 자원과 협동하는 구체적 정보처리에 이용될 때만 발명). ② **신규성** — 다중 공개: 공개 GitHub에 `ontology_service.py` 최초 커밋 **2026-05-21**(`b19af19`), 위원회 발표 2026-07-28(이벤트엣지)·08-24(V4.7), 플랫폼사 handoff 전달(09-10~). 저장소가 언제부터 public이었는지 특정이 어려운 점 자체가 리스크 — 05-21 공개로 보면 공지예외 기한은 **2027-05-21**. **대안**: A안 종속항("온톨로지 정경 준수 검증")으로 편입(반영됨) · 아이디어 1·2(병합 보류·추정 금지 게이트)와 결합한 **방법** 청구 · 설계 문서는 편집저작물·영업비밀로 보호 |
